---
title: "From Linear Regression model to Polynomial Regression model"
date: 2016-08-26
layout: post
...
Tối thiểu \\(\mathbf{J(\theta)}\\) để thu được hàm số dự đoán tốt nhất trong mô hình hồi quy tuyến tính bằng các giải phương trình \\(\mathbf{J'(\theta)} = 0 \\). Với hàm số \\(\mathbf{J(\theta)}\\) được xây dựng như trên chỉ là hàm bậc 2 nên đạo hàm của nó chỉ là hàm bậc 1 nên phương trình có nghiệm duy nhất. Một phương pháp khác là giảm dần giá trị \theta theo độ dốc của hàm số (giải thuật **Gradient Descent**). Dạng của \\(\mathbf{J'(\theta)}\\) như sau: 
...
Trong đó \\(\theta_j\\) là giá trị tại hàng \\(j\\) của vector \\(\theta\\).\\
