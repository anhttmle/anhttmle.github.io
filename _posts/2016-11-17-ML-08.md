---
title: "Understanding Neural Network 2/3"
date: 2016-11-17
layout: post
...
Phép logic ứng với bảng chân lý trên được gọi là **XNOR** — biểu thức logic là \\(y = x_1 \text{ XNOR } x_2 = (x_1 \text{ AND } x_2) \text{ OR } ((\text{NOT } x_1) \text{ AND } (\text{NOT } x_2))\\). Nếu như sử dụng Logistic Regression ta sẽ phải thêm những thành phần đa thức của \\(x_1, x_2\\) vì Decision Boundary ở đây là đường cong, nhưng nếu sử dụng ANN ta có thể không cần thêm feature nào. Lưu ý rằng đây là ví dụ ANN được **thiết kế thủ công** để minh hoạ khả năng biểu diễn logic — trên thực tế ANN sẽ học các trọng số này từ dữ liệu. Cụ thể, ta sẽ thiết kế ANN với các thành phần tương ứng với các phép logic trong đẳng thức XNOR.
