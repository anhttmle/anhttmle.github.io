---
title: "Solving Classification Problem with Logistic Regression 1/2"
date: 2016-09-23
layout: post
...
Bên cạnh đó **Logarit** của một số nguyên dương có giá trị trong khoảng [-\infty , +\infty], nên nếu lấy **logarit** của odds (được gọi là **logit**) ta sẽ có một đại lượng biến thiên như \\(\theta ^Tx\\) (trong mô hình Linear Regression). Chú ý rằng khi input là vector thì \\(\theta\\) cũng là vector, nên \\(\theta x\\) phải được viết là \\(\theta^Tx\\) (nghĩa là matrix chuyển vị \\(\mathbf{\theta}\\) nhân với vector \\(\mathbf{x}\\), vector là một trường hợp đặc biệt của matrix).
...
Trong bài viết sau chúng ta sẽ xây dựng \\( \mathbf{J(\theta)} \\) theo một cách khác. Hẹn gặp lại các bàn trong bài viết tiếp theo !