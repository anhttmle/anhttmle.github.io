---
title: "Regularization – Overfitting and Underfitting"
date: 2016-12-12
layout: post
...
\Big]
+ \frac{\lambda}{2m} \sum_{\ell=1}^{L-1} \sum_{i=1}^{s_{\ell}} \sum_{j=1}^{s_{\ell+1}} \left(\theta_{ji}^{(\ell)}\right)^{2}
$$

\\(L\\): Số lượng layer (Input, Hidden & Output Layer).\\
\\(S_\ell\\): Số lượng neuron của layer \\(\ell\\)
