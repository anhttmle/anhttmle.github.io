---
title: "Machine Learning Diagnostic 2/2"
date: 2017-01-20
layout: post
...
– Nếu \\(\mathbf{\lambda} \\) quá lớn: giá trị của tất cả \\(\theta\\) ngoại trừ \\(\theta_0\\) đều giảm mạnh, xấp xỉ về 0. Khi đó giá trị của hàm hypothesis chỉ còn phụ thuộc vào \\(\theta_0\\) -> High bias -> Underfitting ngay trong tập training.
Chẳng hạn \\(\lambda\\) = 100000, \\(\theta_1  \\approx 0, \theta _2 \approx 0 … h _{\theta} (x) \approx \theta_0 \\)
...
Giờ chúng ta sẽ đi quan sát Learning curves để thấy được số lượng dữ liệu training ảnh hưởng tới kết quả học của máy như thế nào ?
...
Khi gặp phải **Overfitting** (High Variance): \\(J_{train}\\) nhỏ nhưng \\(J_{cv}\\) lại lớn. Ở thời điểm hiện tại, mô hình của ta học tốt trên tập training, có thể dự đoán tốt trên tập training nhưng trên tập CV vẫn còn chưa tốt, tuy nhiên \\(J_{train}\\) và \\(J_{cv}\\) vẫn đang có xu hướng tiến lại gần nhau khi \\(m\\) tăng. Nghĩa là trong trường hợp này, khi tăng số lượng dữ liệu có lẽ sẽ giải quyết được vấn đề. (Khi \\(m\\) đủ lớn, sai số trên 2 tập sẽ gần như bằng nhau). Tuy nhiên, khi tăng số lượng dữ liệu chỉ có thể có tác dụng chứ không chắc chắn vì ngoài ra ta còn gặp các vấn đề khác như \\(\lambda\\) quá nhỏ, bậc mô hình quá cao…
...
* Tăng \\(\lambda\\): fix **High Variance** (tránh việc bỏ qua Regularization)
Đối với ANN:
